...
class RegularizedFM:
    ...
    def __init__(self, lmbda=100, resolvant_gamma=0.5, bidirectional=False):
        super(RegularizedFM, self).__init__()
        self.lmbda = lmbda
        self.resolvant_gamma = resolvant_gamma
        self.bidirectional = bidirectional

    def run(self,feat_x,feat_y,point_x,point_y):
        frames_x, mass_x, L_x, evals_x, evecs_x, gradX_x, gradY_x = compute_operators(point_x, None, 256, None)
        frames_y, mass_y, L_y, evals_y, evecs_y, gradX_y, gradY_y = compute_operators(point_y, None, 256, None)
        evecs_trans_x = evecs_x.T * mass_x[None]
        evecs_trans_y = evecs_y.T * mass_y[None]

        Cxy, _ = self.compute_functional_map(feat_x.unsqueeze(0), feat_y.unsqueeze(0),evals_x.unsqueeze(0),evals_y.unsqueeze(0), evecs_trans_x.unsqueeze(0), evecs_trans_y.unsqueeze(0))    # 1,k,k
    ...
    def compute_functional_map(self, feat_x, feat_y, evals_x, evals_y, evecs_trans_x, evecs_trans_y):
        ...
        """
        A = torch.bmm(evecs_trans_x, feat_x)  # [B, K, C] 将顶点特征投影到谱域中
